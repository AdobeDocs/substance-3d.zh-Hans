...
<table>
  <tr style="border: 0;">
    ...
    <td style="border: 0;" valign="top">钢、铝、铜或金等材料的行为与非金属（电介质）材料截然不同。 对于金属，外观几乎完全由Specular反射驱动：与介质不同，金属没有散射分量，光线不会在表面下方散点，而是直接反射。 它们的定义特点是：<br><br><ul><li>无漫射分量 — 颜色完全来自反射</li><li>彩色Specular反射</li><li>表面细节，尤其是粗糙度，在外观中起着主要作用</li></ul><br><br><strong>材料的关键参数：</strong><br><br><ul><li>base color控制反射的颜色</li><li>粗糙度可控制这些反射的锐化或模糊程度</li><li>Specular量缩放反射强度</li></ul></td>
  </tr>
</table>

### 基底金属度

基础金属度定义材料的行为是作为电介质还是金属 — 这不仅仅是一种视觉调整，而且是材料的基础光响应中的变化。

* **0**→完全不金属（扩散+Specular）
* **1**→完全金属（仅限Specular）
* **0-1**→两种行为的混合。 中间值最适合材料混合物，如Dirt、腐蚀或磨损表面，而不是“部分金属”材料。
...
对于绘画或涂层金属、透明和材料，使用分层（如涂层）而不是降低金属含量。

### 透明和材料

透明和材料允许光线穿过它们。 常见示例包括玻璃、许多液体以及透明或有色塑料。 它们的定义特点是：

* 光线进入曲面，离开反面
* Thickness强烈影响外观
* 受折射率(IOR)控制且受曲面粗糙度影响的折射率
* 最终外观的折射、吸收、散布和色散形状

透射率描述光如何穿过物体。 较粗的区域看起来较暗或较饱和，而较细的区域看起来更清晰。 “传输颜色”、“传输深度”、“散点颜色”和“色散”等参数共同控制此行为。

“透明”和“transmissive”这两个词的区别在于：“透明”是现实生活中的日常用语；如果我们能看穿它，一切都是透明的。 “Transmissive”是“translucency”的同义词。 例如，磨砂玻璃可以让光线穿过它（因此，它是transmissive的），但它并不透明 — 我们看不见它穿过。

### 次表面材料

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/extra/subsurface-scattering/subsurfaceScattering.png" alt=""/><br><em>使用次表面散射的材料示例。</em></td>
    <td style="border: 0;" valign="top">次曲面材料允许光线进入曲面、在其下方散点并再次在入口点附近退出。 常见的例子包括皮肤、蜡、大理石和许多有机材料，例如许多类型的食物。  — 水果、蔬菜，或者圣内克泰尔奶酪。 次曲面材料的定义特点是：<br><br><br><ul><li>柔和的扩散着色</li><li>薄区域中的渗色</li><li>外观取决于Thickness</li><li>光线不会穿过对象</li></ul><br><br><br>次表面散射与传输不同。 透射描述通过材料并离开对面的光，而次表面散射描述进入表面、在该表面内散射、然后离开其进入的点附近的光，大多数是在同一侧。 请注意，材料不支持传输或次表面散射。 更改完全材料（即，“基础金属度”值为1的材料）的透过率或次表面值不会影响其外观。</td>
  </tr>
</table>
...
像PBR材料中的粗糙度参数一样，OpenPBR材料中的Specular粗糙度表示微观表面变化：即使是肉眼看起来光滑的表面也具有散点反射光的微小瑕疵。 此值可重现该效果，通过定义光的反射清晰度或广度，控制曲面在其反射中出现的平滑度或粗糙度。 低粗糙度的材料将产生锐利的、类似镜像的反射。 相反，具有较高粗糙度的材料将产生柔和的模糊反射。

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness01.png" alt=""/><br><em>粗糙度= 0.1</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness05.png" alt=""/><br><em>粗糙度= 0.5</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/specular/roughness/roughness08.png" alt=""/><br><em>粗糙度= 0.8</em></td>
  </tr>
</table>
...
IOR描述材料与光相互作用的程度，既控制光线进入材料时的弯曲（折射）方式，又控制光线如何呈现（反射）效果，尤其是在浅视角（掠射）时更是如此。 反光度较低的表面（如水或某些塑料）具有低的IOR。 反射性更强的表面 — 如玻璃或某些宝石 — 将具有更高的IOR和更强的折射效果。 材料的IOR是一个物理值，因此它是一个客观的数字，而不是一个艺术解释问题。 在创建给定材料时，您只需查找材料的IOR，并确保其设置正确，以确保材料能够正确地响应光线。 有一系列信息源可以在线列出材料的IOR。 例如，花岗岩的IOR是1.43；如果要创建花岗岩材料，可输入此值作为其IOR，这将确保光线以真实的方式反映您的材料。 请注意，IOR与金属的材料无关（请参阅下面的“金属性”）。 更改材料的IOR值不会影响其外观。
...
当微观表面变化以相同方向排列时，材料反射将趋向于依赖于观察方向且垂直于沟槽拉伸。 这些沟槽越对齐，效果越明显。 材料的各向异性值定义曲面的反射在所有方向上是否显示相同，或者它们是否以特定方式拉伸。 这可能会重现金属刷等材料的效果，例如，画笔效果沿着“画笔效果”产生的反射要长得多。 当用指纹涂抹抛光表面或者当诸如干皮的可变形表面拉伸时，也可以以更细微的方式发生各向异性反射。
...
当存在一定程度的各向异性（即，材料的各向异性值大于0）时，各向异性正切指示沟槽的主方向。 反射将与该方向垂直拉伸。
...
OpenPBR还包括影响材料与几何交互方式的参数，如不透明度和薄壁行为。 这些控制决定一个表面应被视为具有物理Thickness还是薄壳，这对于纸张、树叶、窗户或织物等材料尤其重要

+++几何参数

* **薄壁**：启用薄壁后，材料在微观上被视为很薄。 光线被认为是在没有可见折射的情况下通过材料的。
* **不透明度**：确定是否可以通过材料部分或全部查看。 请注意，虽然Transmission参数定义了材料的透明度，但Opacity参数可用于定义净额结算，也就是说使用“删除”材料信息来创建孔。

+++

### 基底图层

在OpenPBR模型的底部，Base层表示光照与表面材料本身之间的基本交互。 基本层由四个特性定义：基本重量、Base color、金属度和Diffuse粗糙度。
...
* **基本粗细**：基本定义Base color的强度（如下所示），范围从0到1，值为0时主要生成黑色（无材料），值为1（尽可能使用最强的红、绿和蓝光）。
...
* **Base color**：这确定材料的“主色”，并设置金属基色和漫射（对于非金属）基色的反照率，即红、绿和蓝光的反射量。 如上所述，虽然Base color决定了反射的颜色，但“基准权重”设置决定了此反射的强度。
...
* **金属度**：定义材料在0-1标度（0 =电介质，1 =完全金属和不透明）上的行为是非金属还是金属。
...
<table>
  <tr style="border: 0;">
    ...
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/metalness/metalness1Colored.png" alt=""/><br><em>金属度= 1.0，带黄base color</em></td>
  </tr>
</table>

* **粗糙度**：定义材料的微表面粗糙度，范围从0（具有非常光滑、均匀的反射）到1（具有非常粗糙的漫反射），适用于岩石或树皮等材料。

<table>
  <tr style="border: 0;">
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/diffuse-rough/diffuseRoughness0.png" alt=""/><br><em>粗糙度= 0.0</em></td>
    <td style="border: 0;" valign="top"><img src="../assets/openpbrf/renders/base/diffuse-rough/diffuseRoughness1.png" alt=""/><br><em>粗糙度= 1.0</em></td>
...
*使用地下通道的材料。 请注意手部和网格的其他细小区域中的translucency。*

+++次曲面参数

* **次表面粗细**：这定义了使用多少次表面散射 — 本质上，有多少光线进入材料。
...
* **子表面颜色**：定义从材料表面下方重新出现的任何光线的整体颜色。 较亮的颜色通常会产生更明亮、更明显的散射；而此处的黑色值则不会产生任何次表面散射效果。
...
* **次表面半径**：定义在散射或吸收之前，光可在材料内传播的距离。 如果使用较低的值，则光线仅会传输很短的距离；因此，材料外观会较为密集。 使用较大的半径时，光线可以传播得更远；材料将具有柔和、蜡质和半透明的外观。
...
* **子表面半径比例**：控制平均自由路径的颜色通道依赖性。 换句话说，光在被吸收或散射之前，在每个RGB通道中独立地穿过材料的距离。 这将产生在地下通道中可见的特征材料变化：在网格的较薄区域中，光线移动较短距离，颜色向具有最长半径的通道偏移。

默认值(1， 0.5， 0.25)表示红光传播最深，其次是绿色，然后是蓝色，这与许多真实世界子表面材料（包括皮肤）的行为非常匹配。
...
* **次表面各向异性**：定义光线在次表面材料内偏好散点的方向。 值为0时，光照将在所有方向均匀散点。 如果为正值，光线将趋向于向前散点，方向与初始光线相同；这通常可导致材料外观更清晰、半透明。 在负值下，光线将趋向于向后散点到光束的来源；这通常会使材料看起来更不透明、更密。
...
“透射”可控制通过材料的光量。 与次表面不同，透射控制穿过对象的光线总量，而次表面控制从对象内部反射回表面的光线的总量。
...
* **粗细**：控制可以透过材料表面的光量。 通常用于液体或玻璃等透明材料。
...
* **深度**：以厘米为单位定义一束光线在透射色达到完全饱和之前必须穿过材料的距离 — 本质上，是当光线穿过透明（或部分透明）材料时，它拾取颜色的速度有多快。 对于传输深度较低的材料，光线将很快拾取颜色，这意味着，即使是材料很细的部分看起来也是颜色强烈的。 相反，如果具有较高的深度，较厚的部分将显得很暗或几乎不透明，并且材料的外观将具有“浓缩”效果，如彩色树脂或浓稠液体。
...
* **散点颜色**：此颜色定义在透明或部分透明的材料中散射的光线的强度和颜色。 它实质上定义了材料的内部“混浊度”，从而确定了光线在材料内的扩散和柔和方式。 散点可用于再现光线不能干净或沿直线传播的材料，如某些塑料、牛奶或阴沉的苹果汁，甚至可用于再现大量的水（如创作海洋的蓝色调）。
...
* **各向异性**：这决定了光线在材料内部的散点方向。 值为0时，光照将在所有方向均匀散点。 如果为正值，光线将趋向于向前散点，方向与初始光线相同；这通常可导致材料外观更清晰、更像玻璃。 在负值下，光线将趋向于向后散点到光束的来源；这通常会使材料看起来更结霜或粉笔。
...
>[!NOTE]
>
> 各向异性取决于光方向，因此这种散射的结果将随光源的放置位置（相对于被照亮的材料）而变化。

* **色散(Abbe)**：此选项用于定义光线经过透明材料时，会弯曲多少种不同的颜色，从而产生分色、彩虹状条纹或折射光线中的彩色边缘。 如果色散(Abbe)值为0，则会完全禁用此效果。 低色散(Abbe)值将导致非常可见的颜色分离（正如您在棱镜中看到的那样），而高色散(Abbe)值将导致较弱或可忽略的颜色分离，以及整体上更清晰、更清晰的折射。 (色散(Abbe)参数以19世纪物理学家和光学工程师恩斯特·阿贝命名。 )
...
* **传输色散**：与其他地方的粗细参数一样，此值定义了材料内的光色散强度。 这种情况在高对比度折射的边缘最为明显。
...
“发射”可控制材料是否发射自己的光（与反射光无关），并允许您设置发射光的强度和颜色。
...
* **明亮度**：定义从材料发出的光线的亮度，测量单位为cd/m²，也称为nit。 此测量假定为白光；更改光线的颜色（请参阅下文）可能会影响整体亮度。
...
* **Thickness**：定义胶片图层的Thickness（以微米为单位）。 在物理精确的材料中，大多数薄膜效果发生在0到1微米之间的Thickness。
...
* **折射率(IOR)**：如上所述，材料的IOR决定了材料与光线的反应程度。 材料的薄膜层具有自己的IOR。 例如，菱形的IOR为2.417。
...
* 折射率(IOR)：实质上是根据光线在涂层内的行为方式，非金属表面如何呈现的数值定义。
...
* 粗糙度：如讨论基底层时所述，表面粗糙度定义表面的反射方式 — 光滑表面非常均匀地反射光，而粗糙表面在随机方向散点光。 “Coat”（涂层）图层将具有其自身的粗糙度程度。
...
>[!NOTE]
>
> 请注意，即使“基本”图层很平滑（即，其粗糙度值接近0），但“涂层”图层的粗糙度也可能会使整体材料显得更粗糙。

* 各向异性：各向异性描述涂层反射如何因方向而异，从而使高光沿表面拉伸或对齐，而不是呈圆形。 此效果用于表示涂层中的定向表面结构，如刷涂、划痕或流型。
...
* coat normal：涂层可以发生微小的变形，从而形成精细的几何形状。 例如，该功能可用于再现材料上的划痕或雨滴外观。
...
* **粗糙度**：实质上决定此图层内“模糊粒子”的形状。 当该值接近0时，粒子高而细；当从浅（掠过）角度查看曲面时，它们更明显。 在较高的值下，粒子会变得更接近球面；它们从更宽的角度范围内更容易看见，因此表面整体会显得更粗糙。
...
本节重点介绍使用现代统一的PBR模型（如OpenPBR）创建强大、可预测材料（可在光照条件、场景和工具间正常工作）的实际指导。 也就是说，下面许多建议一般都适用于PBR材料创建；尽管如此，一些建议仍取决于OpenPBR材料的特定功能集。

### 从真实参考资料开始

基于物理的材料基于真实世界的观察时最可靠。 尽可能地基础材质对摄影参考、测量值或类似表面的直接观察做出决定。 这不仅适用于粗糙度，也适用于颜色、反射率和表面变化。 根据参考操作有助于在合理的范围内定位材料，从而使它们更易于重复使用且对光照或环境变化不那么敏感。 这样还可以降低对材料内部光线问题进行补偿的诱惑。

### 对材料的物理结构有一个心理模型

OpenPBR不仅仅是一个参数列表，艺术家可以对这些参数列表进行微调，直到获得想要的外观。 其核心依赖于“材料各层概述”中描述的基础结构，该结构假定一个材料由类似的物理分层结构组成。 因此，在考虑材料的同时，建议使用OpenPBR参数描述这些材料的物理元素。 想象一下材料是由什么组成的 — 在显微镜下垂直切片会是什么样子，颜色和亮部来自哪里，等等。 尽可能多地尝试预期需要哪些OpenPBR组件才能实现这种期望的外观。 同样，也可以尝试另一种方法 — 即从一组图层建立材料，并发现其最终外观。

### 独立于光照创作材料

PBR工作流程的一个关键强度是分离材料和光照之间的关注。 材料应描述表面属性，而不是补偿场景光照、曝光或情绪。 旨在创造在多种光照条件（甚至恶劣的光照）下都保持稳定且可信的材料。 这种分离使场景更易于管理、调试和迭代 — 特别是在可能需要不同艺术家处理材料和照明的较大管道中。 在一系列上下文中验证材料可能非常有用。 一个创作良好的材料应能在不同的光照环境、比例和相机角度下保持光照。 如有可能，在多个上下文中预览材料 — 例如，在中性摄影棚照明下和较戏剧性的场景中。 这有助于了解材料的外观是否真正基于其参数，或者它是否依赖于特定设置来看起来正确。 在上下文中经过良好验证的材料更易于重复使用，在生产环境中也更加可靠。

### 尽可能保持参数解耦

现代的PBR工作流程旨在最小化参数之间的隐藏相关性。 调整值（如粗糙度、金属量或传输量）时，目标应仅影响材料外观的特定方面。 在实践中，这意味着：

* 除非有明确的物理理由，否则避免从单个纹理中驱动多个视觉效果。
* 与紧密互连的网络相比，更偏爱简单、可读的参数设置。
* 以增量方式作出更改，尽可能单独评估其影响。 此方法使材料更易于理解、更易于调试，并且当在其他环境中重复使用时更可预测。

### 有意使用图层

分层材料功能强大，但也会增加复杂性。 每增加一个图层都会增加视觉和计算成本，并会使材料更难推理。 分层时：

* 使用图层来表示实际的表面结构（例如，材料顶部的Dust或Dirt）。
* 避免栈叠可产生类似视觉效果的图层。
* 定期评估图层是否对最终外观做出有意义的贡献。 捕捉曲面基本特征的简单材料通常比难以控制的高层文档更稳健。

### 注意性能、噪声和稳定性

某些材料功能和组合本质上更昂贵或更容易噪声，尤其是在路径跟踪渲染器中。 材料中使用的功能越多，渲染的成本可能越高。 地下、高粗糙度结合传输、多层效果、各向异性或色散都可以增加渲染时间和方差。 虽然这些功能很有价值，但使用时应该谨慎从事 — 具体取决于艺术家的设置，它们可能会造成过多的噪声、不稳定或长时间的渲染时间。 了解使用高级功能的成本，并在这些功能提供明确视觉价值的情况下使用这些功能非常重要。

### 有意偏离物理合理性

虽然在物理上看似合理的数值提供了一个强有力的基准，但生产现实有时需要有意的偏离。 风格化、可读性、艺术方向或技术约束可能使将参数推到超出实际范围的范围是合理的。

根据具体项目、材料和艺术意图，具体情形会大相径庭，而承认这些时刻本身就是一个判断问题，而不是遵守规则的问题。 重要的是偏离是有意和有目的的：你明白自己背离了什么物理原理，以及为什么这样做对工作有益。
...
物理工作流程中的一个常见缺陷是，将材料视为预定义的“外观”，而不是对光线行为的描述。 这通常显示为严重依赖预设或复制参数值，而不理解它们所代表的含义。

OpenPBR围绕显式的光相互作用 — 反射、透射、散射、吸收和发射。 当材料看起来不正确时，最有效的故障排除方法是确定这些行为中的哪些，然后直接进行调整。 与循环切换预设或栈叠效果相比，这样可以做出更清晰的决策，获得更可预测的结果。

### 使用Specular粗细代替Specular粗糙度

要控制材料的反射率，可以先调整Specular权重，但更建议先调整Specular粗糙度参数。

所有材料都有Specular反射，Specular反射总是在掠过角处达到100%。 此外，大多数介电（非金属）材料具有非常相似的Specular反射，在垂直入射时在2%到8%之间。 视觉反射率差异的主要原因是由于材料的微观几何结构造成的，这由Specular粗糙度参数来定义。

但是，“Specular粗细”仍然很有用，可以用作局部调整折射率、模拟微遮蔽导致的反射率变化或后期艺术调整的速记。

### 混淆传输、透明度和次表面散射

光通过效果通常松散地分组在“透明”或“半透明”下，但OpenPBR可以清楚地区分它们。 透射是指穿过材料并出射到另一侧的光线，如玻璃、水或透明塑料中看到的光线。 次表面散射描述光进入材料、在内部散射以及在不同点退出，从而产生柔和阴影和内部颜色。
...
在传输更合适的位置使用次表面散射（反之亦然）可能会导致材料过于复杂且渲染效率低下。 OpenPBR会分离这些行为，以便艺术家可以选择最匹配其引用的行为，或者在需要时有意组合这些行为。

### 添加没有明确视觉动机的特征

由于公开了广泛的材料行为 — 包括涂层、模糊、薄膜效果、次表面散射和发射 — 因此可能很容易同时启用多个特征。 如果在添加时没有明确的“参考驱动”原因，则会使材料更难控制，并且视觉噪点较大。
...
在不同光照条件下测试材料可以揭示它们是稳定的还是过度依赖于场景。 考虑到此灵活性而创作的材质往往会更顺畅地跨不同环境和项目集成。

### 使用无参照的极端参数值

尽管OpenPBR参数是按物理含义建立的，但将它们推至无明确目的的极端值会导致不稳定或混淆结果，尤其是在光照发生变化时。 当材料的行为无法预测时，将参数选择与真实参考进行对比，可以帮助确定问题是出于艺术意图还是参数滥用。 使用引用建立基准决策可使材料在整个项目中更易于诊断、优化和维护一致。

### 误解模型的局限性

并非所有材料都可以用OpenPBR表示。 和任何材质模型一样，OpenPBR只是一个模型。 尽管它已经拥有相当丰富的特色，但与现存或人们所能想象到的巨大而丰富的材料相比，它仍然粗糙。 模型可以表示一些现成的材料，一些需要更多经验来构建，将模型拉伸到极限，还有一些仍然是模型无法表示的。 在某些情况下，熟练的艺术家仍然可以通过一些“作弊”获得不错的结果；这通常是在做出非身体选择的时候。 但了解模型能做什么和不能做什么，以及何时需要替代解决方案（如更简单的材料或专用的着色器）非常重要。

### 期望材质模型解决渲染问题

并非所有视觉问题都源自材料本身。 光照、取样或渲染器设置（而不是材料定义）可能会导致噪声、收敛缓慢或着色伪影。

虽然OpenPBR可提供物理上一致的材质模型，但它并不取代对适当光照和渲染配置的需求。 隔离变量（例如，在简化光照下测试材料）有助于确定问题出在材料中还是其他地方。

### 将预设用作学习工具，而不是最终答案

最好将“OpenPBR预设”理解为“参考工具”和“学习工具”。 检查预设值（如金属量、粗糙度、各向异性或传输深度）有助于阐明具体视觉结果的构建方式。

依赖预设作为最终解决方案可能会遮盖材料的实际运作方式。 使用这些模板作为起点或分析示例有助于加深理解并创建更适合的材料。
...
基于物理的渲染(PBR)是一种围绕一个简单想法构建的渲染方法：材料应以与现实世界表面行为一致的方式响应光线，而不是依赖于特定的光照设置。 PBR材料旨在使多种环境保持可信，使其更可预测、可重复使用且更易于在现代生产管道中管理。

这种真实世界基础的一个直接后果是PBR工作流程允许艺术家根据实际测量值复制现实，而不是试图猜测其最佳近似值。 在光照中，这可能意味着使用物理单位和真实世界的强度，而不是任意值。 在与拍摄内容或影片内容集成的渲染工作流程中，基于物理的相机和着色器可帮助保留真实镜头和传感器的视觉特性。 对于材料，相同的原理支持摄影测量等技术，其中扫描的表面可以与手动创作的材料无缝混合，因为使用相同的物理假设来描述两种材料。

对于艺术家，PBR在工具、引擎和渲染器之间提供共享的视觉语言。 使用PBR原则创建的材料无论是在实时引擎、路径跟踪渲染器还是在明显不同的光照条件下进行查看，都旨在看起来连贯一致，而无需不断手动调整。 这种一致性是PBR成为游戏、VFX和可视化标准的关键原因。

PBR的核心是一些关于光和表面的基本物理思想。 光被看作是反射、散点或被表面吸收的能量，而着色器被设计来保存这些能量，以便材料不会显得不自然的明亮或反射。 微观粗糙度等因素影响表面形貌，从而影响反射的锐利度和柔软度。 PBR工作流程还清晰地区分金属和非金属，因为这些材料类型与光的相互作用方式截然不同。 PBR依赖于描述物理属性（例如base color、粗糙度和金属度）的参数，而着色器使用物理推导出的模型进行解释。

同样重要的是，PBR提高了渲染过程不同部分之间的低互依性。 通过将材料定义与光照分离，艺术家们不必在每次光线发生变化时“修复”材料。 这种划分可以将复杂的问题转变为更小、更易于管理的问题：光照可以独立于材质进行调整，材质可以在不知道最终场景设置的情况下创作。 在更细微的尺度上，现代的PBR模型（包括OpenPBR）旨在保持参数尽可能独立，使艺术家能够单独调整值，而不会导致意外的副作用。

在实践中，PBR将艺术家的角色从补偿光照或渲染器古怪，转变为根据现实世界特征描述材料。 结果就是一种优先于场景特定微调的工作流程，从明确的素材输入中自然而然地呈现真实感，而不是手工制作的光照技巧。

有关PBR技术细节的详细信息，请参阅[PBR指南，作者：Wes McDermott](https://www.adobe.com/learn/substance-3d-designer/web/the-pbr-guide-part-1)。

## 附录二：什么是OpenPBR？

OpenPBR是一种开放的、基于物理的表面着色模型，旨在提供一致且可预测的方式以描述跨不同3D工具、渲染器和管道的材料外观。 它定义了一个单一、全面的材质模型，可以表示广泛的真实世界表面，同时仍然保持使用物理上有意义的参数描绘更奇特的或艺术上习惯的表面的灵活性。
...
这些限制在3D社区中随处可见，艺术家、工作室和开发人员开始寻找解决方案。 最初的方法各有不同，整个社区的这一持续努力逐渐趋同于共同解决方案。 这项工作，以及围绕这项工作的许多讨论和联合决定，都在一个材料创建统一方法下正式确定：OpenPBR是一种常见的、公开文档化的材质模型，可以跨应用程序一致地实施，而不是与单个软件绑定，OpenPBR建立在不同的工具可以构建的共同基础之上，同时保留相同的底层物理行为。 这种通用模型使艺术家可以更轻松地在应用程序之间转移材料，使工作室可以标准化外观开发做法，并使资源在制作过程中保持视觉稳定。 最重要的是，OpenPBR从根本上说是一种共识；即使在今天，讨论仍在进行，并且在决策时也寻求来自3D部门众多专家的共识。

模型本身是基于物理渲染(PBR)原则的。 这意味着材料是以光线与真实世界中的表面相互作用的方式来描述的，强调节能以及对光照的可预测响应，参数植根于真实世界的光学，这些参数的组织和公开方式支持实际外观的发展而不是科学模拟。 即，OpenPBR定义材料本身的行为 — 参数的含义、不同图层交互的方式以及材料在光照下的响应方式。 单个软件工具可以自由地以不同的方式展示这些控件，使用任何看起来最合适的UI样式，只要基础材质模型保持一致即可 — 尽管实际上，参数的命名、分组和排序背后有一个逻辑，而特定应用程序通常倾向于遵守这一点。
...
这需要大量迭代，而3D场景最初非常零散，各个利益相关者探索表达视觉效果的不同方式，并在不同方面取得进步。 一个解决方案将被另一个解决方案取代，直到具体的方法变得优越，各个领域的工作开始趋同，导致GGX、金属粗糙材料方法的出现，并最终出现OpenPBR。

与此同时，生产管道也变得更加互联了。 在建模、添加纹理、外观开发、照明、渲染和实时使用等应用程序之间移动所需的资产越来越多。 工作室开始更多地依赖USD和MaterialX等标准化交换格式，并且很明显，允许特定的材料描述移动的格式也将有利。
...
OpenPBR背后的一个关键动机是一致性。 此处的目标是确保使用OpenPBR描述的材料无论在何处都以可预知的方式行事，而不牺牲艺术控制或创意灵活性。 当艺术家调整粗糙度、金属性或Specular响应时，预期这些更改将在合规性实现中具有相同的视觉含义。

另一个重要的动机是耐用性。 通过作为行业标准公开指定和管理，OpenPBR旨在随时间不断变化，而不受单个产品或公司的生命周期或优先级限制。 这使得其成为长期资源创建的一个更稳定的基础，特别是对于工作室和艺术家，他们希望随着材料的改变，其工具保持可用和相关性。