...
breadcrumb-title: ''
description: 了解Substance Bakers如何处理正切空间计算并为您的工作流程自定义算法。
...
Substance Bakers可以加载低多边形网格上显示的正切和二项式，也可以重新计算它们。 重新计算它们时，可以定义自定义切线空间算法（默认情况下为MikkTSpace）。

## 切线空间的增效工具列表

## Substance Painter

在Substance Painter中，无法更改切线空间插件，它将始终为&#x200B;**MikkTSpace**。 但是，有一个参数可以略微改变其行为，使其与其他应用程序兼容：

| *参数* | *兼容* *应用程序* |
| --- | --- |
| **计算每个片段的正切空间：已禁用** | 与xNormal、Unity 5.3或更高版本兼容。 |
| **计算每个片段的正切空间：已启用** | 与Unreal Workflow 4、Blender和Unity HDRP引擎兼容。 |
...
| **mikkunrealtspace.dll** | Miktspace，基于Morten S. Mikkelsen的切线空间算法。与Unreal Workflow 4、Blender和Unity HDRP引擎兼容。 |
| **unitytspace.dll** | 基于Unity 4的切线空间算法。 |

>[!NOTE]
>
> 可以编写自定义切线空间插件。 名为&#x200B;**tangentspaceplugin.h**&#x200B;的标头文件位于&#x200B;**Substance Designer/SDK/tangentspace**&#x200B;下的安装文件夹中，可以用作接口。
