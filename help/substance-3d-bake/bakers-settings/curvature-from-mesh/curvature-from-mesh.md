...
| **自动色调映射边界** | 控制应如何将弯曲值写入纹理。 如果启用，将根据烘焙过程中找到的最小值和最大值在0和1之间规范化值范围。 如果禁用，则手动定义最小值和最大值。  **注意：**&#x200B;在UDIM/UV 平铺时，应禁用此参数以使色调映射统一而不是特定于每个拼贴，否则可能会在每个纹理之间创建接缝。 要手动查找正确的最小值/最大值，请先烘焙启用此设置，然后查看控制台/日志以查看Baker输出的值。 |
| **色调映射最小值** | 如果禁用了&#x200B;**自动色调映射边界**，请定义最小值以缩放弯曲结果以适合纹理。 |
| **最大色调映射** | 如果&#x200B;**自动色调映射边界**&#x200B;已禁用，请定义最大值以缩放弯曲结果使之适合纹理。 |
