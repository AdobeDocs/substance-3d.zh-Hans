...
* 解决了以下问题：当石板材料编辑器打开且选择了Substance2材料映射时，无法使用Corona在IR/Production中渲染Substance2纹理。
* 解决了连接器更新创建新的Substance2节点而不是更新现有节点的问题。
* 解决了添加Substance2崩溃时3ds Max增效工具中的节点问题，并确保使用批量导入加载.sbsar文件不再打开脚本编辑器。
...
* 通过将所有现有图标合并到qrc和rcc文件中，实现Substance增效工具中的标准化图标管理，与Autodesk的首选方法一致，并确保在SBSAR图形面板中加载的一致性。
* 提高了增效工具中“Substance设置”窗口的响应能力，以确保在调整窗口大小时输入字段及其描述能够正确匹配。
* Substance增效工具现在与Corona 11兼容。

<b>已修复：</b>

* 解决了光泽颜色和光泽粗糙度在V-Ray材料中未自动连接的问题。 现在，在V-Ray和Arnold中创建工作流程时，这两个属性都将自动链接。
