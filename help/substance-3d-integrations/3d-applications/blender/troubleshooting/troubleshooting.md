...
* *更新加载项并加载材料后，并非所有设置都显示在Substance 3D面板中。*
  * 如果在同一会话中移除较旧版本的加载项并安装较新版本，则可能会发生这种情况，因为较旧的文件可能仍会缓存在系统中。\
    重新启动Blender应使更改生效。
* *安装加载项。/材料时出现问题，处理停滞在会话之间。 /材料不在会话之间生成纹理。 /加载.sbsar文件时出错。*
...
* *材料未在循环渲染视图*&#x200B;中更新。
  * 默认情况下，该插件不会更新“循环”渲染视图中的纹理。 但是，可以通过在加载项首选项中启用<b>循环自动更新纹理</b>来强制更新它们。
* 在“循环”渲染视图中存储后，参数显示为要恢复的状态。
  * 这是搅拌器端的已知缓存问题，仅供查看。 保存时，不会向远程引擎发送任何消息以更新生成的纹理文件。 离开“循环”渲染视图并切换回视图后，纹理将正常显示。
* *在撤消/更改参数后，材料不再更新。*
  * 在撤消操作后，材料可能无法更新。 虽然参数将恢复到以前的状态，但纹理无法撤消以匹配。 要再次更新纹理，请使用“刷新”按钮将参数恢复为默认值并重新加载纹理。
* *在Blender的拾色器中，以Substance Designer设置的颜色略有不同，颜色值也不相同。*
  * 混合器仅对混合器的拾色器的颜色应用灰度系数校正。 虽然这会导致拾色器不一致，但在纹理中显示的颜色与Substance应用程序中设置的值准确。
* 在Windows中加载材料时，*出现“无法识别wmic”控制台错误。*
...
* *使用“刷新”按钮或更新参数时，对图形的修改会撤消。*
  * 该加载项在更改或刷新后刷新图形中的连接。 要解决此问题，请复制从.sbsar创建的混合器材料，然后根据需要为其指定一个新名称。 仅将节点添加到副本中。 在保留用户添加的节点的同时，纹理将在节点组中更新。 刷新时，复制这些节点，并在刷新后将其粘贴回新图形。
