...
<b>拼贴</b> — 调整材料的X、Y、Z拼贴。 锁定可用于取消链接各个值并单独对其进行调整。

<b>分辨率</b> — 所生成纹理的默认分辨率。 锁可用于取消链接以单独设置其分辨率。

<b>应用类型</b> — 设置“应用”按钮的行为： <b>插入</b>将使用选定的材料覆盖当前Substance，<b>追加</b>将材料添加到新材料槽中的对象。

<b>导出图像格式</b> — 如果在Blender中生成的图像用作材料的图像输入，此格式用于将该图像保存到临时文件夹。

<b>默认折叠输入组</b> — 切换材料的输入组是否默认展开或折叠。

<b>默认情况下仅更新纹理</b> — 切换天气更新Substance参数只会影响Blender着色网络中的输出纹理。 禁用此选项将在调整参数后重置节点连接。 建议在向材料中添加其他节点时启用，否则在调整参数后将断开这些节点。
