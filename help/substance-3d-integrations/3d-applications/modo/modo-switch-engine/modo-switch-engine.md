...
breadcrumb-title: ''
description: 在MODO中切换CPU和GPU引擎，以根据硬件优化性能。
...
该Substance 引擎有两个版本，即CPU和GPU。 GPU引擎用于创建高于2K的纹理。 CPU引擎只能生成最多2K的纹理。 如果需要更高分辨率的纹理，则需要切换到GPU引擎。

转到“Substance工具包”菜单中的“Substance设置”选项，然后选择“切换Substance 引擎”。 您需要重新启动MODO才能启用GPU引擎。 此设置用作全局首选项。 然后，每次运行MODO时都会启用GPU引擎，直到手动切换它为止。

>[!NOTE]
>
> **使用SubstanceGPU引擎需要具有1GB或更大专用视频RAM的GPU。 不支持集成的GPU。**\
