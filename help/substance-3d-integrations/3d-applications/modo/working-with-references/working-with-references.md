...
breadcrumb-title: ''
description: 在MODO中管理Substance素材引用，以便在多个对象和场景之间共享素材。
...
Substance素材可用于参照场景。 但是，如果需要禁用被引用场景中Substance的输出，则需要手动移除输出。 使用MODO的默认引用首选项时，仅取消选中Substance上的输出不会删除所引用属性的输出。\
要允许参照的Substance材料删除其自身的生成输出要手动移除输出，必须首先更改场景的“参照覆盖”。 转到“项目”>“引用”>“编辑引用覆盖”并将“删除”设置为“如果项目允许”。 这将允许您手动删除Substance输出\
从着色器树和剪辑浏览器中查找在进行此更改后打开或创建的任何场景。
