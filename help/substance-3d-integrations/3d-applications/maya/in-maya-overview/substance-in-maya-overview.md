...
Substance增效工具允许您直接在Maya中加载在Substance Designer中创建的Substance材料。 该增效工具将创建Maya材料并将Substance纹理馈送到材料频道输入中。 然后，您可以更改Substance参数，纹理将自动更新。
...
1. 打开Hypershade，然后在节点编辑器中，右键单击标记菜单并向上滑动以选择创建节点。 此时将打开“创建节点”窗口。 在此处，您可以搜索Substance节点。
   ...
   还可以在节点编辑器中按Tab键，然后在文本字段中键入substance，这样即可筛选到substance选项。 从选项中选择纹理。
1. 在“属性编辑器”中选择Substance节点并浏览以加载Substance(.sbsar)文件。

   ![](../../../assets/1.png)
1. 如果Substance包含多个图形，则会填充所选图形下拉列表。 将使用所选图形创建材料。
1. “图形信息”按钮将显示在Substance Designer中设置的图形属性。
