...
breadcrumb-title: ''
description: 了解如何转换Substance素材输出以匹配不同的渲染器要求和工作流程。
...
Substance增效工具将生成输出并自动为特定工作流创建材质。 但是，如果使用DCC应用程序和第三方渲染器，则可能需要手动转换金属/粗糙输出。 以下集成支持自动渲染工作流程，并且将在需要时相应地转换任何映射类型：
...
如果要构建自定义Substance，则可以创建渲染器所需的特定输出，例如Vray和Corona。 使用金属/粗糙度转换节点（“库”>“PBR实用程序”），您可以轻松地将基色、粗糙度和金属贴图转换为特定的渲染器。
