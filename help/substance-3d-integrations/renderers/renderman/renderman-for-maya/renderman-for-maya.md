---
helpx_url: "https://helpx.adobe.com/substance-3d-integrations/renderers/renderman/renderman-for-maya.html"
breadcrumb-title: ''
description: 在Maya中使用pxrSurface或pxrDisney素材进行PBR渲染，同时使用Renderman和Substance素材。
helpx_creative_field: ""
helpx_description: Ecosystems and Plugins > Renderers > Renderman > Renderman for Maya
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: 玛雅的渲染
user-guide-description: ''
user-guide-title: ''
source-git-commit: 0197b6f5f4e3ed1f2bc0e5576bd5818d04485ed5
workflow-type: tm+mt
source-wordcount: '55'
ht-degree: 0%

---


# 玛雅的渲染

## Maya增效工具中的Substance

MayaSubstance中的插件通过Renderman渲染工作流程支持pxrSurface。 使用此工作流程将创建pxrSurface着色器，并转换Substance输出以用于素材。

![](../../../assets/renderman-1.png)
