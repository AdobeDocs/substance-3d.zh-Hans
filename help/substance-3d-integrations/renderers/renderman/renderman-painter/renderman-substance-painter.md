...
breadcrumb-title: ''
description: 使用pxrSurface材质和适当的输出转换为Renderman导出Substance Painter纹理。
...
| SpecularFaceColor | 主要Specular/面部颜色 |
| 法线 | 全局/凹凸/PxrNormalMap →方向(Open GL) |
| 位移 | （红色通道） PxrDispTransform （结果F） → （显示标量） PxrDisplace (Out Color) → （位移着色器） PxrSurfaceSG |
