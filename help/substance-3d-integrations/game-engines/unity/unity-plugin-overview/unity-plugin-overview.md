...
1. 在“项目”窗口中右键单击，然后选择“导入资源”，或将要导入的材料拖动到“项目视图”面板中。
1. 浏览要导入的材料。 材料的文件扩展名为“.sbsar”。
1. 材料将导入到您的Unity项目中。

   1. sbsar资源将创建一个主导入文件以及一个包含输出纹理和生成的Unity材料的文件夹。
1. 然后，可以在“场景”视图的网格上拖放该材料，然后在“检查器”中编辑参数。
...
> **法线图转换**
> 
> Unity增效工具中的Substance将自动将DirectX转换为OpenGL。 使用[Substance Source](https://source.substance3d.com/)中的材料时，不需要将法线方向更改为OGL。 如果要在Substance Designer中创建自己的材料，请确保使用默认DirectX着色器，因为此增效工具将自动处理普通转换。 有关更多信息，请查看在Unity中使用法线。
