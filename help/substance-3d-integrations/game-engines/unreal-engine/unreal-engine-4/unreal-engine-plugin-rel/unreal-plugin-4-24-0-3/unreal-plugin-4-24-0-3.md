...
Unreal Engine的Substance经历了重大重组。 此重构的一部分提供了&#x200B;**UTexture2D**&#x200B;的完全支持，并且包括输入和输出。 借助&#x200B;**UTexture2D**&#x200B;支持，该插件现在可用于发布到任何平台，包括移动设备在内的虚实支持。 除了添加多个平台支持外，**UTexture2D**&#x200B;还允许在UE4中本地使用纹理流系统。

该增效工具还完全支持素材&#x200B;**实例化**，并引入了一个新的素材模板Substance 引擎，其中具有模板支持的数字输出。 材质模板允许您确切定义要在UE4中配置Substance材质着色器的方式。
